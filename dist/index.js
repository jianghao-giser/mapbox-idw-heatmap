 ...
 */
function createDrawFragmentShader(valueToColor, valueToColor4) {
    return `
    precision highp float;
    ${valueToColor}
    ${valueToColor4}
...
// 主类
// ============================================================================
class MapboxInterpolateHeatmapLayer {
    // ==========================================================================
    // 构造函数
    // ==========================================================================
    constructor(options) {
        ...
        // 处理后的数据
        // --------------------------------------------------------------------------
        this.points = []; // [x_mercator, y_mercator, normalized_val]
        // --------------------------------------------------------------------------
        // WebGL 资源 - 计算阶段
        // --------------------------------------------------------------------------
        this.computationProgram = null;
        this.computationFramebuffer = null;
        this.computationTexture = null;
        this.computationVerticesBuffer = null;
        this.uMatrixComputation = null;
        this.uUi = null;
        this.uXi = null;
        this.uP = null;
        this.uFramebufferSize = null;
        this.uMercatorBounds = null;
        // --------------------------------------------------------------------------
        // WebGL 资源 - 绘制阶段
        // --------------------------------------------------------------------------
        this.drawProgram = null;
        this.drawingVerticesBuffer = null;
        this.uMatrixDraw = null;
        this.uComputationTexture = null;
        this.uDataBounds = null;
        this.uOpacity = null;
        // --------------------------------------------------------------------------
        // WebGL 资源 - 共享
        // --------------------------------------------------------------------------
        this.indicesBuffer = null;
        this.indicesNumber = null;
        this.indexType = WebGLRenderingContext.UNSIGNED_BYTE; // 索引类型
        this.id = options.id || '';
    ...
    prerender(gl, matrix) {
        ...
        if (this.mercatorBounds && this.uMercatorBounds) {
            ...
            gl.uniform4f(this.uMercatorBounds, this.mercatorBounds.minX, this.mercatorBounds.minY, this.mercatorBounds.maxX, this.mercatorBounds.maxY);
        }
        // 绑定帧缓冲
    ...
    render(gl, matrix) {
        ...
        if (this.mercatorBounds && this.uDataBounds) {
            ...
            gl.uniform4f(this.uDataBounds, this.mercatorBounds.minX, this.mercatorBounds.minY, this.mercatorBounds.maxX, this.mercatorBounds.maxY);
        }
        else {
    ...
    initShaderPrograms(gl) {
        ...
        if (!compVS || !compFS)
            throw new Error('Computation shader compilation failed');
        this.computationProgram = createProgram(gl, compVS, compFS);
        if (!this.computationProgram)
            throw new Error('Computation program creation failed');
        // 获取计算程序的 locations
        ...
        if (!drawVS || !drawFS)
            throw new Error('Draw shader compilation failed');
        this.drawProgram = createProgram(gl, drawVS, drawFS);
        if (!this.drawProgram)
            throw new Error('Draw program creation failed');
        // 获取绘制程序的 locations
    ...
    initBuffers(gl, drawingVertices) {
        // 绘制顶点缓冲
    ...
    initFramebuffer(gl) {
        ...
        if (isWebGL2) {
            const gl2 = gl;
            gl2.texImage2D(gl2.TEXTURE_2D, 0, gl2.RGBA32F, this.framebufferWidth, this.framebufferHeight, 0, gl2.RGBA, gl2.FLOAT, null);
        }
        else {
...
exports.MapboxInterpolateHeatmapLayer = MapboxInterpolateHeatmapLayer;
